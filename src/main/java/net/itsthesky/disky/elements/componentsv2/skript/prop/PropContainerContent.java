...
public class PropContainerContent extends MultiplyPropertyExpression<ContainerBuilder, Component> {

    static {
        register(
                PropContainerContent.class,
                Component.class,
                "content",
                "container"
        );
    ...
    public Class<?> @NotNull [] acceptChange(@NotNull Changer.ChangeMode mode) {
        if (EasyElement.equalAny(mode, Changer.ChangeMode.ADD))
            return new Class[]{
                    String.class, TextDisplayBuilder.class,           // Text displays
                    SectionBuilder.class,   // Sections
                    SeparatorBuilder.class, // Separators
                    FileDisplayBuilder.class, // File displays
                    MediaGalleryBuilder.class, // Media galleries
                    ComponentRow.class
            };
        return new Class[0];
    }
    ...
    public void change(@NotNull Event e, @NotNull Object[] delta, @NotNull Changer.ChangeMode mode) {
        ...
        for (Object comp : delta) {
            if (comp instanceof final IContainerComponentBuilder<?> builder)
                container.addContent(builder);
            else if (comp instanceof final String text)
                container.addContent(new TextDisplayBuilder(text, -1));
        }
    }
    ...
    @Override
    protected Component[] convert(ContainerBuilder containerBuilder) {
        return containerBuilder.getComponents().stream()
