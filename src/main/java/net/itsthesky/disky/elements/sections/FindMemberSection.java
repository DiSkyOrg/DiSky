...
@Examples({"find members in event-guild and store them in {_members::*} with filter var {_m}:", "\t# {_m} now contains the member to apply the filter to. For instance:", "\tif {_m} has discord role with id \"XXX\":", "\t\treturn true", "reply with \"I have found %size of {_members::*}% that has the role!\""})
@Since("4.14.3")
...
public class FindMemberSection extends Section implements ReturnHandler<Boolean> {
            ...
            if (getNext() != null) {
                Bukkit.getScheduler().runTask(Skript.getInstance(), () -> {
                    ...
                    if (SkriptTimings.enabled()) {
                        ...
                        if (t != null) {
                            timing = SkriptTimings.start(t.getDebugLabel());
                        }
                    }
    ...
    public @NotNull String toString(@Nullable Event event, boolean debug) {
        return "find members in guild " + exprGuild.toString(event, debug)
                + " and store them in " + exprResult.toString(event, debug)
                + " with filter " + exprValue.toString(event, debug);
    }
}